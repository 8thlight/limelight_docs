...
def adjust_scrollbar
  if selected_child_below?      
    self.peer.vertical_scrollbar.value = (@selected + 2) * 18 - 300
  elsif selected_child_above?
    self.peer.vertical_scrollbar.value = (@selected - 1) * 18
  end
end

def selected_child_below?
  (@selected + 2) * 18 > (300 + self.peer.vertical_scrollbar.value)
end

def selected_child_above?
  (@selected - 1) * 18 < self.peer.vertical_scrollbar.value
end
