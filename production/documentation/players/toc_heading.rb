...
on_mouse_clicked do
  unless already_selected? || disabled?
...
def build_new_toc_links_on(arbitrary_wrapper)
  arbitrary_wrapper.build(:partial_name => "#{self.id.downcase}_links.rb") do
    __install "#{Dir.pwd}/production/documentation/#{@partial_name}"
  end
end
...
def clear_content_pane
  ...
  content_pane.build(:id => self.id) do
    __install "#{Dir.pwd}/production/documentation/#{@id}_content_pane.rb"
  end
...
def clear_search_results
  ...
  scene.remove(search_results_screen) if search_results_screen
end
