slide do
  heading :text => "Images"
  directions :text => "You first saw images in the Players tutorial.  Images have a few quicks that are worth exploring here, so lets get to it."
end
...
slide do
  ...
  directions :text => "Hmm, that wasn't very good.  Lets see how we can fix it.  Images have a height and a width property like any prop, and when they are set to auto the image will appear as the actual size of the image.  However images can also be scaled to a size of your choosing.  In this example you'll set the scaled property to true - and set the height to fit this window.  Notice how the image's width is adjusted to keep the aspect ratio of the image, because the width is scaled to fit as well."
...
slide do
  ...
  directions :text => "Images can also be rotated using the rotation property.  Let's let poor Homer lie down.  Note that Negative is counter-clockwise, so naturally positive is clockwise."
  sandbox_codeblock do
