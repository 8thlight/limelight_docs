slide do
  ...
  line_break
  directions :text => "By default, props have float set to off.  If you turn float to on, the rest of the props will be placed on the screen as if the floating prop was not there.  Then the floating prop will be placed on top of the rest of the props."
end

slide do
  heading :text => "Simple Example"
  directions :text => "Once you set turn float :on, you can then use the x and y style attributes to move the prop around.  x and y can takes values representing a number of pixels or a percentage of the size of the parnet prop."
  line_break
...
slide do
  ...
  directions :text => "If multiple props have float on, the last prop placed will appear on the top of the screen.  Try swapping these two props."
  sandbox_codeblock do
